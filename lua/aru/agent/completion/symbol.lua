...
function M:get_completions(completion_context, callback)
    local ref = require("aru.agent.reference").at_cursor(
        completion_context.line,
        completion_context.cursor[2]
    )
    ...
    local symbols = require("aru.agent.context").symbol_candidates(ref, cwd, invocation_buf)
