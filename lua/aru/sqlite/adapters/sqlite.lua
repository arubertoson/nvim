---@module "aru.sqlite.adapters.sqlite"
...
---@class aru.sqlite.AdapterResult
...
---@class aru.sqlite.ExportResult
---@field error string|nil
---@field execution_ms number

---@class aru.sqlite.Adapter
---@field db_path string
local Adapter = {}
Adapter.__index = Adapter

---@class aru.sqlite.CsvParser
...
---@param parser aru.sqlite.CsvParser
...
---@param sql string
---@param callback fun(result: aru.sqlite.AdapterResult)
---@return vim.SystemObj
function Adapter:execute(sql, callback)
    local started_at = vim.uv.hrtime()
    local null_value = ("__ARU_SQLITE_NULL_%x__"):format(started_at)
    local command = {
        "sqlite3",
        "-batch",
        "-csv",
...
---@param callback fun(result: aru.sqlite.ExportResult)
...
function Adapter:export(sql, path, overwrite, callback)
    ...
    local temporary_path = ("%s.aru-%x.tmp"):format(path, started_at)
