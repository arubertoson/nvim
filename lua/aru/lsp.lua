...
local function map_buffer_keys(bufnr)
	...
	map("fS", function() fzf.lsp_live_workspace_symbols() end, "Workspace symbols")
	map("fd", function() fzf.lsp_document_diagnostics() end, "Buffer diagnostics")
	map("fD", function() fzf.lsp_workspace_diagnostics() end, "Workspace diagnostics")
	map("gd", function() fzf.lsp_definitions() end, "Goto definition")
	map("gr", function() fzf.lsp_references() end, "References")
	...
	map("<leader>tv", function()
		...
		if type(vt) == "table" then
			...
			enabled = vt ~= false and vt ~= nil
		end
		if enabled then
			vim.diagnostic.config({ virtual_text = false })
		else
			vim.diagnostic.config({ virtual_text = diagnostic_config.virtual_text })
		end
...
local function setup_inlay_hints(client, bufnr)
	...
	local group = vim.api.nvim_create_augroup(("aru_inlay_hints_%d"):format(bufnr), { clear = true })
...
local function arm_gc(client)
	...
	timer:start(gc_ttl, 0, function()
		...
		if current and not has_loaded_buffers(current) then
			pcall(current.stop, current, true)
		end
	end)
