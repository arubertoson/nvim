...
local T = MiniTest.new_set({
    hooks = {
        pre_case = function()
            require("aru.sqlite.session").close()
...
T["adapter"]["configures sqls for its database"] = function()
    ...
    local config = require("aru.sqlite.adapters.sqlite").new(db_path):lsp_config()
...
T["adapter"]["streams quoted CSV and discards records beyond its limit"] = function()
    local helpers = require("aru.sqlite.adapters.sqlite")._test
...
T["render"]["renders successful execution without a result set"] = function()
    local lines = require("aru.sqlite.render").result({
...
T["session"]["executes asynchronously without moving query focus"] = function()
    ...
    MiniTest.expect.equality(vim.b[query_buf].aru_sqlite_query, true)
    MiniTest.expect.equality(vim.api.nvim_buf_get_name(query_buf), "")
    MiniTest.expect.equality(
        vim.t[scratchpad_tab].aru_tab_label,
        "SQLite: " .. vim.fs.basename(db_path)
    )
    MiniTest.expect.equality(
        require("aru.sqlite.ui").tabline():find("SQLite:", 1, true) ~= nil,
        true
    )
